...
And the development version of the package can be installed with:
```bash
pip install -e .
```
...
## Algorithms
An SMC sampler for Bayesian Decision Trees is implemented. Working examples for the implemented variable types can be found in the `examples` directory.
